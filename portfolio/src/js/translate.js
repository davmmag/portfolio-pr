...
const loadLanguage = (activeLang = 'en') => {
  const activeName = 'language-switch__btn--active';
  const lang = localStorage.getItem('lang') || 'en';
  let target = document.querySelector(`[data-lang="${activeLang}"]`);
  if (activeLang === lang && !target.classList.contains(activeName)) {
    changingActiveElement(target, activeName);
  }
  if (activeLang !== lang ) {
    changingActiveElement(target, activeName);
    translateElements(activeLang);
  }
}
